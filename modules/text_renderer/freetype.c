...
#define SAVE_BP(dst) do { \
        ...
        while( i_index < i_start + i_length )
        {
            ...
            if( !p_previous_style || p_previous_style->i_font_size != p_current_style->i_font_size ||
                (p_previous_style->i_style_flags & STYLE_HALFWIDTH) != (p_current_style->i_style_flags & STYLE_HALFWIDTH) )
            {
                int i_font_width;
                int i_font_height;
                if( p_current_style->i_style_flags & STYLE_HALFWIDTH )
                {
                    i_font_width = p_current_style->i_font_size / 2;
                }
                else
                {
                    i_font_width = p_current_style->i_font_size;
                }
                i_font_height = p_current_style->i_font_size;
            ...
            while( i_part_length > 0 )
            {
                ...
                FT_Vector pen_shadow_new = {
                    ...
                    .y = pen_new.y + p_sys->f_shadow_vector_y * (i_font_height  << 6),
                };
                FT_Glyph glyph;
                FT_BBox  glyph_bbox;
                FT_Glyph outline;
                         ...
                         const vlc_fourcc_t *p_chroma_list )
{
    ...
    else
    {
        ...
        {
            p_style = CreateStyle( (p_region_in->p_style->psz_fontname) ? p_region_in->p_style->psz_fontname : p_sys->psz_fontfamily,
