...
struct decoder_sys_t
{
    ...
    int               i_horizontal_adjustment_value;
    int               i_vertical_adjustment_value;
...
static int Open( vlc_object_t *p_this )
{
    ...
    p_sys->b_replace_ellipsis = p_sys->b_use_coretext;
    p_sys->i_horizontal_adjustment_value =
        (p_sys->b_use_coretext) ? 12 : 0;
    p_sys->i_vertical_adjustment_value = 0;
#ifdef ARIBSUB_GEN_DRCS_DATA
...
static void load_drcs_conversion_table( decoder_t *p_dec )
{
    ...
    while( fgets( buf, 256, fp ) != 0 )
    {
        ...
        if( code > 0x10ffff )
        {
            msg_Err( p_dec, "code[%lx] is invalid: hash[%s]psz_code[%s]",
                    code, hash, psz_code );
            continue;
        }
...
static subpicture_t *render( decoder_t *p_dec, block_t *p_block )
{
    ...
    if( p_sys->b_a_profile )
        arib_initialize_decoder_a_profile( &p_sys->arib_decoder );
    else
        arib_initialize_decoder_c_profile( &p_sys->arib_decoder );
    if( !p_sys->b_use_coretext )
        p_sys->arib_decoder.b_use_private_conv = false;
    ...
    for( int i = 0; i < p_sys->i_drcs_num; i++ )
    {
        ...
        while( p_drcs_conv != NULL )
        {
            if( strcmp( p_drcs_conv->hash, p_sys->drcs_hash_table[i] ) == 0 )
            {
                uc = p_drcs_conv->code;
                break;
            }
        ...
        if( uc )
        {
            msg_Dbg( p_dec, "Mapping [%s=U+%04x] will be used.",
                p_sys->drcs_hash_table[i], uc );
        }
        else
        {
            msg_Dbg( p_dec, "Mapping for hash[%s] is not found.",
                p_sys->drcs_hash_table[i] );
        }
#endif
        p_sys->arib_decoder.drcs_conv_table[i] = uc;
    }

    p_sys->arib_decoder.b_replace_ellipsis = p_sys->b_replace_ellipsis;
    ...
    char* psz_subtitle_data_hex = (char*) calloc(
            p_sys->i_subtitle_data_size * 3 + 1, sizeof(char) );
    ...
    p_spu->i_stop = p_block->i_pts + (int64_t)
        (p_sys->arib_decoder.i_control_time * 1000000 / 10);
         ...
         p_buf_region; p_buf_region = p_buf_region->p_next )
    {
        ...
        p_region->i_charleft_adj = p_sys->i_horizontal_adjustment_value;
        p_region->i_charbottom_adj = p_sys->i_vertical_adjustment_value;
