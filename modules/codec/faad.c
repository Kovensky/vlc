...
static block_t *DecodeBlock( decoder_t *p_dec, block_t **pp_block )
{
    ...
    if( ((p_dec->fmt_out.audio.i_rate == 0) ||
                ((p_dec->fmt_in.audio.i_rate != p_dec->fmt_out.audio.i_rate)&&(p_dec->fmt_in.audio.i_rate*2!=p_dec->fmt_out.audio.i_rate)) ||
            ...
            p_sys->i_buffer && !b_inited)
    {
        ...
        msg_Dbg( p_dec, "init1-2: %d, %d (rate %d=%d chan %d=%d)", i_channels, i_rate ,p_dec->fmt_in.audio.i_rate,p_dec->fmt_out.audio.i_rate,p_dec->fmt_in.audio.i_channels,p_sys->input_channels);
